...
- `balanceCents`: extract it whenever the SMS states the wallet balance. The optional balance check that stops faked SMS depends on it; receipts without a balance are held for review while that check is on.
